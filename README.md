...
例题：[http://hihocoder.com/problemset/problem/1441?sid=1601206](http://hihocoder.com/problemset/problem/1441?sid=1601206)
...
例题：[https://oj.bashu.com.cn/code/problempage.php?problem_id=4037](https://oj.bashu.com.cn/code/problempage.php?problem_id=4037)

```cpp
#include<cstdio>
const int MAXN=1000005;struct node{int len,son[26],link;}st[MAXN];int sz,last;int c[MAXN],p[MAXN];long long cnt[MAXN];template<typename T>void swap(T&a,T&b){T t=a;a=b;b=t;}template<typename T>inline int strlen(T*s){for(int i=0;;i++){if(s[i]==0)return i;}}int mmp[MAXN],mmmp;void sam_extend(char cc){int c=cc-'a';int cur=++sz;int p=last;last=cur;st[cur].len=st[p].len+1;cnt[cur]=1;while(p!=0&&!st[p].son[c]){st[p].son[c]=cur;p=st[p].link;}if(p==0){st[cur].link=1;}else{int q=st[p].son[c];if(st[p].len+1==st[q].len){st[cur].link=q;}else{int nq=++sz;st[nq].len=st[p].len+1;for(int i=0;i<=25;i++){st[nq].son[i]=st[q].son[i];}st[nq].link=st[q].link;while(p!=0&&st[p].son[c]==q){st[p].son[c]=nq;p=st[p].link;}st[q].link=st[cur].link=nq;}}}char str[MAXN];int main(){last=1;sz=1;scanf("%s",str+1);int n=strlen(str+1);for(int i=n;i;i--){sam_extend(str[i]);}for(int i=1;i<=sz;i++){c[st[i].len]++;}for(int i=1;i<=sz;i++){c[i]+=c[i-1];}for(int i=1;i<=sz;i++){p[c[st[i].len]--]=i;}long long ans=0;for(int i=sz;i;i--){cnt[st[p[i]].link]+=cnt[p[i]];ans+=(st[p[i]].len-st[st[p[i]].link].len)*cnt[p[i]]*(n-cnt[p[i]]);}printf("%lld",ans);return 0;}
```

例题：[http://hihocoder.com/problemset/problem/1445](http://hihocoder.com/problemset/problem/1445)

```cpp
#include<cstdio>
const int MAXN=2000005;struct node{int len,son[26],link;}st[MAXN];int sz,last;template<typename T>void swap(T&a,T&b){T t=a;a=b;b=t;}template<typename T>inline int strlen(T*s){for(int i=0;;i++){if(s[i]==0)return i;}}int mmp[MAXN],mmmp;void sam_extend(char cc){int c=cc-'a';int cur=++sz;int p=last;last=cur;st[cur].len=st[p].len+1;while(p!=0&&!st[p].son[c]){st[p].son[c]=cur;p=st[p].link;}if(p==0){st[cur].link=1;}else{int q=st[p].son[c];if(st[p].len+1==st[q].len){st[cur].link=q;}else{int nq=++sz;st[nq].len=st[p].len+1;for(int i=0;i<=25;i++){st[nq].son[i]=st[q].son[i];}st[nq].link=st[q].link;while(p!=0&&st[p].son[c]==q){st[p].son[c]=nq;p=st[p].link;}st[q].link=st[cur].link=nq;}}}char str[MAXN];int main(){last=1;sz=1;scanf("%s",str+1);int n=strlen(str+1);for(int i=n;i;i--){sam_extend(str[i]);}long long ans=0;for(int i=2;i<=sz;i++){ans+=st[i].len-st[st[i].link].len;}printf("%lld",ans);return 0;}
```
